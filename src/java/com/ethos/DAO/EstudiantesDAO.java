...
public class EstudiantesDAO extends AbstractDAO<EstudianteModel> {
    ...
    public boolean update(EstudianteModel entityClass) {
        ...
        try {
            conn = getConnectionDB().getConnection();
            query = EstudiantesQuery.QUERY_ACTUALIZAR_DATOS_COMPELEMTARIOS_ESTUDIANTES;
            psQuery = conn.prepareStatement(query);
            psQuery.setInt(1, Integer.parseInt(entityClass.getDatosEstudianteModel().getPeriodoAcademico()));
            psQuery.setInt(2, Integer.parseInt(entityClass.getDatosEstudianteModel().getFondoPensiones()));
            psQuery.setInt(3, Integer.parseInt(entityClass.getDatosEstudianteModel().getPaisSecundaria()));
            psQuery.setInt(4, Integer.parseInt(entityClass.getDatosEstudianteModel().getCategoriaSisben()));
            psQuery.setInt(5, Integer.parseInt(entityClass.getDatosEstudianteModel().getDepartaSecundaria()));
            psQuery.setInt(6, Integer.parseInt(entityClass.getDatosEstudianteModel().getNomEscSecundaria()));
            psQuery.setInt(7, Integer.parseInt(entityClass.getDatosEstudianteModel().getCiudadSecundaria()));
            psQuery.setInt(8, Integer.parseInt(entityClass.getDatosEstudianteModel().getEps()));
            psQuery.setInt(9, Integer.parseInt(entityClass.getDatosEstudianteModel().getFechaGraduacion()));
            rs = psQuery.executeUpdate();
